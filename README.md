...
**Phase 4 — Complete Game Simulation is in progress.** Phase 3 is complete. `GameSession` now separates campaign and encounter ownership, preserves full-vs-encounter reset semantics, continuously ticks the recovered player/projectile/shield/bomb/cadence/scroll helpers, and owns all 17 trajectory groups. The common live mode-2 wave activation, stagger/path/escape lifecycle, and proven-hit damage/destruction/score/group teardown now execute through the whole-session boundary. Template-selection randomness, exact opaque-pixel hit detection, non-trajectory encounters, Drone transitions and death/post-game continuity remain current Phase-4 work.
