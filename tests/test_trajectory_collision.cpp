...
int main() {
    ...
    // pixels miss; a nonzero palette index consumes the missile and adds +3.
    {
        ...
        assert(result.destroyed_actor_event_count == 1);
        assert(result.destroyed_actors[0].group_index == 1);
        assert(result.destroyed_actors[0].actor_index == 0);
        assert(result.destroyed_actors[0].x == 10);
        assert(result.destroyed_actors[0].y == 20);
        assert(result.destroyed_actors[0].sprite_width == 10);
        assert(result.destroyed_actors[0].sprite_height == 10);
        assert(result.destroyed_actors[0].destruction_burst_count == 2);
    ...
    // destruction intentionally preserves the actor's retained damage byte.
    {
        ...
        assert(result.destroyed_actor_event_count == 2);
        assert(result.destroyed_actors[0].group_index == 1);
        assert(result.destroyed_actors[0].actor_index == 0);
        assert(result.destroyed_actors[0].x == 50);
        assert(result.destroyed_actors[0].y == 60);
        assert(result.destroyed_actors[1].group_index == 1);
        assert(result.destroyed_actors[1].actor_index == 1);
        assert(result.destroyed_actors[1].x == 50);
        assert(result.destroyed_actors[1].y == 60);
    ...
    // original local+mission hit increments on a destruction.
    {
        ...
        assert(tick.trajectory_destroyed_actor_event_count == 1);
        assert(tick.trajectory_destroyed_actor_events[0].group_index == 1);
        assert(tick.trajectory_destroyed_actor_events[0].actor_index == 0);
        assert(tick.trajectory_destroyed_actor_events[0].x == 80);
        assert(tick.trajectory_destroyed_actor_events[0].y == 90);
