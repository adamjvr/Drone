...
- The fourteen transient trajectory `squad1.wav`..`squad14.wav` families are fully initialized in metadata: each is a 20-voice DirectSound pool at game volume 80, source/default frequency, with exact loader/string/pool storage ranges cataloged and regression-tested.
...
- The portable original-policy voice runtime now executes lowered commands without sample bytes or OS audio APIs: Win32 reusable cues have independent 20-slot first-free/slot-0-steal pools, DOS has one global 32-voice first-free/fail-on-saturation array, generation-checked completion prevents stale callback corruption, and dedicated/retained control ownership plus DOS digital-master state are regression-tested.
- The host-independent PCM mixer/render core is now integrated above that runtime: canonical signed-16-bit mono/stereo samples render through Q32.32 cursors, integer linear interpolation, backend-native sample/master gain, exact full-sample loop semantics, overlap summing and signed-16-bit saturation; natural one-shot completion feeds the generation-safe voice runtime without any platform device API.
...
| portable audio-event boundary | ESTABLISHED lowering + voice-runtime + PCM-render contract | allocation-free semantic events carry typed control domains; backend lowering preserves original topology/loop/volume/stop semantics; host-independent execution models Win32 per-cue 20-slot steal-on-saturation pools and DOS global 32-voice fail-on-saturation state; the deterministic PCM layer now adds sample registration, Q32.32 cursor/rate stepping, loop wrap, native gain/master application, overlap summing, clipping and natural-completion retirement |
...
| DOS HMI / presentation semantics | EXECUTABLE RUNTIME + LIFECYCLE CONTRACT ESTABLISHED | Canonical DOS configures 32 x 0xF0 voices; StartSample selects first inactive and returns -1 on saturation; packed HMI sample volume and UINT32_MAX loop count are exact; Lowbees menu ownership is state-mapped; states 5/6/99 attenuate the HMI digital master while Air continues; gameplay teardown SampleDone/stops Air before menu return. Q-AUDIO-003..007 are resolved and the portable voice runtime now executes this policy without OS audio binding |
...
2. **Host-independent voice + PCM mixer: COMPLETE first runtime.** Lowered commands now execute against faithful abstract voice policy and deterministic signed-16-bit PCM rendering with fixed-point cursors, looping, rate control, gain/master stages, overlap summing and natural completion.
3. Add original-asset import/decode adapters and bind this validated runtime to Linux/macOS/Windows/iPadOS device APIs without moving device latency or callback scheduling into deterministic gameplay state.
