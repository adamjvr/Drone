...
| death/restart/post-game continuity | PARTIAL Phase-4 integration | bomb-driven player destruction entry, -540 spawn quiet period, native 42×38 death-explosion pre-roll/visible/terminal lifecycle, and exact deferred life-consumption/respawn/game-over gate are session-owned; randomized debris/audio/pixels and remaining results execution are still external |
...
3. Continue whole-frame integration into remaining encounter producers, complete randomized player-death/Drone-detonation presentation ownership, and finish the remaining non-alien results continuity without pulling retail-only unknowns forward from Phase 8.
