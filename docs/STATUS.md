...
| portable audio-event boundary | ESTABLISHED lowering + voice-runtime contract | allocation-free semantic events carry typed control domains; backend lowering preserves original topology/loop/volume/stop semantics; host-independent execution now models Win32 per-cue 20-slot steal-on-saturation pools and DOS global 32-voice fail-on-saturation state with generation-safe completion and retained controls |
