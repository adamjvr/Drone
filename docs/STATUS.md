...
| encounter actor integration | CURRENT | all 17 trajectory groups, persistent group-0 replenishment, exact live transient formation timing/template/path randomization, encounter-local alien hit/total statistics with the original mission double-count/fold quirks, and shareware boss state are session-owned; native trajectory weapon collisions are connected; Lid/Top now owns root movement, bomb emission, lid open/close vulnerability and boss-local missile/Stinger hits; Gemini movement/local damage and remaining non-trajectory enemies remain to be connected |
...
1. Extend established continuous `GameSession` ownership from trajectories, Drone/Probe interaction and native Lid/Top combat into Gemini movement/local damage and the remaining non-trajectory encounter actors.
