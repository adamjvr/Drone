...
- Recovered exact 16.16 root initialization/movement, player-left-X tracking, horizontal acceleration/cap, Y>=240 boundary handling, live phase-2 bomb chance/gate/slot/position behavior, and reused-bomb field preservation. Later Phase-5 audio re-read corrected that boundary from an apparent velocity retreat to a fixed-Y wrap; see the traversal-cadence entry below.
...
- Recovered shared-root player tracking (`±0x44C` X acceleration with difficulty-scaled cap), Y>=240 boundary handling, opposing 30-frame phase-2 body animation, and phase-2 enemy-bomb emission. Later Phase-5 audio re-read corrected that boundary from an apparent velocity retreat to a fixed-Y wrap; see the traversal-cadence entry below. Bomb chance is `rand()%100 < 2*difficulty`; source selection is `rand()%10` with preferred-side fallback, and reused bomb motion/frame fields are preserved.
...
- Added a metadata-only loop-call catalog and extended semantic cue definitions for all four Results tracks plus Ordering/Credits. `GameSession` now emits exact start/stop ownership events across Results confirmation, Ordering completion, high-score-to-credits handoff, and credits completion. The presentation host now additionally executes the credits post-scroll 99..0 volume cadence before that completion stop.
...
## 2026-08-28 — Phase 5 native `air.wav` ambience envelope

- Re-read the active state-2 preamble and shared post-encounter tail and established `0x004729A0` as the process-global game-scale `air.wav` volume scalar. The gameplay loader/start uses volume 50 and looping playback; the main-menu path's separate 11025-Hz override is not part of this gameplay owner.
- Recovered the exact bidirectional envelope around the four-phase scheduler: before scheduler advance, settlement >=60 increments the scalar one per logical update toward 50; after phase-2 settlement advancement, settlement <60 decrements it one per phase-2 update toward zero.
- Confirmed detonation ownership starts by stopping/rewinding `air.wav`, while the shared post-encounter tail explicitly stores zero and performs `StopAndRewind -> Play(loop) -> SetVolume(0)` so the following encounter fades back up from silence.
- Integrated the scalar into `OriginalAudioRuntimeState` and the exact control events into `GameSession`; dedicated whole-session regressions cover +1/cap, phase-2 -1/floor, detonation stop, restart ordering and process-lifetime reset behavior. Raw states 5/6/99 and the menu frequency path remain host/menu reconstruction work rather than guessed gameplay behavior.

## 2026-08-28 — Phase 5 menu/overlay audio host ownership

- Recovered `run_main_menu`'s dedicated `lowbees.wav` owner from `0x00418ADB..0x00418B1C`: process/bootstrap byte `0x00459F8C` arms loading, menu entry explicitly applies volume 0 before flags-1 Play, and consuming the start clears the byte.
- Recovered the exact menu fade at `0x004190F9..0x00419110`: a stack-local game-volume scalar increments by one per menu loop and is applied only while below 80. Cleanup at `0x00419DA6..0x00419DE3` is selective for raw states `0`, `2`, `7`, `13`, and `-1`; it stop/resets then releases `lowbees.wav` and re-arms `0x00459F8C`. Instructions and High Scores do not take that cleanup branch.
- Recovered the post-menu `air.wav` control tail at `0x0041A3D6..0x0041A41C`: every non-zero raw state performs flags-1 Play, stores scalar zero, applies volume 0, then sets frequency 11025 Hz. Raw state 0 skips the restart.
- Recovered the shared pause/quit/nine-lives overlay audio path at `0x0040C521..0x0040C665`: while the process-global air scalar is positive it decrements by one each overlay iteration and applies the new volume; at zero it stop/rewinds the slot. Returning to active state 2 reaches the separate `0x0040C82C..0x0040C913` restart that plays the loop and restores scalar/volume 50.
- Added a clean host-side `presentation_audio` runtime and parameterized `SetFrequency` event. This runtime shares the established process-global air scalar with `GameSession` but intentionally keeps menu/modal cadence out of deterministic gameplay.
- Recovered completion-credits fade execution at Win32 0x00404ACC..0x00404B1F: the scroll phase leaves its local scalar at 100; once the final tracked credit reaches Y <= -40, each fade/presentation iteration decrements first and calls SetVolume with 99..0, exactly 100 writes total, then StopAndRewind occurs at 0x00404B1A before stack-local release at 0x00404B3E.

## 2026-08-29 — Phase 5 shareware boss traversal cadence correction

- Re-read the Lid/Top bottom branch `0x00416885..0x004168C5` while tracing `level1.wav` and corrected an earlier Phase-4 field interpretation: `0x00446E0C` is the root **fixed Y position**, not vertical velocity. At integer Y>=240 the original increments shared byte `0x00454B04`, plays `level1.wav` with flags 0 when the count reaches 8, resets the byte, and writes `-100<<16` to fixed Y while leaving positive descent velocity unchanged. Initializer `0x00417304` clears the cadence byte; the loader gives `level1.wav` volume 90.
- Independently verified Gemini's parallel branch at `0x004050BA..0x004050F9`: body-A fixed Y is `0x00467544`, vertical velocity is `0x0046754C`, count 8 plays handle `0x00466B0C` (`level2.wav`, volume 100, flags 0), fixed Y wraps to -100, and initializer `0x00405F87` clears the same shared byte. This proves repeated downward traversals rather than an upward retreat for both shareware bosses.
- Promoted the traversal counter/wrap into the clean boss lifecycle and emitted allocation-free `LidTopLevel1Cadence` / `GeminiLevel2Cadence` events only on every eighth completed pass. Regression tests lock preserved vertical velocity, fixed-Y reset, counter reset, cue identity/volume/flags, and GameSession event ownership/order.

## 2026-08-29 — Phase 5 Drone one-shot identity closure

- Resolved the transient Y=-40 Drone approach sound by following global slot `0x004D8510` back to the static loader: `0x0041F73B..0x0041F75A` loads literal `hintdron.wav` and applies volume 80; `0x0040E55C..0x0040E56C` is its only playback site and uses Play flags 0 before Y is stored as -39.
- Resolved the successful Probe-decode one-shot: slot `0x0042F1F8` is loaded from `parachut.wav` at volume 60 (`0x0041F810..0x0041F82F`), then `0x0040CEA9..0x0040CEBE` plays it once immediately before stop/rewind of `drone.wav`. The same parachute slot is reused by separate presentation/input paths, so the clean cue is asset-centered rather than globally decode-owned.
- Cross-checked both stems against the preserved Windows/DOS asset inventories (`Sounds/Hintdron.wav` / `HINTDRON.CLV`, `Sounds/Parachut.wav` / `PARACHUT.CLV`). Added semantic cue metadata and exact `GameSession` emission; no original sound payloads are included.
- Split the clean Drone approach landmark result so Y=-117 loop start and Y=-40 hint one-shot are independently observable and regression-tested; decode completion now locks `ParachuteOneShot -> StopAndRewind(drone.wav)` ordering.

## 2026-08-29 — Phase 5 static Squad pool initialization closure
