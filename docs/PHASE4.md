...
2. **Whole-frame simulation integration — IN PROGRESS** — the session now owns all 17 trajectory groups, blue-Probe attachment and exact two-stage decode/disarm timing, rapid-missile/Stinger Drone-hit countdown producers, the canonical normal Drone objective position/travel/hover/disarm/settlement path, and the destructive timeout/countdown/detonation/life-loss path, plus automatic Y=-200 shareware boss dispatch and the recovered lifecycle/score tails for Lid/Top and Gemini. It executes both normal and detonated count-1 Gemini continuation, count-2 shareware Results/EndRun transitions, the exact detonation tick/effect-settlement gates, and the original life-decrement ordering. Random trajectory/template selection, trajectory sprite-mask collision producers, attached-Probe vulnerability/enemy collision handling, hostile Stinger target selection, direct detonation visuals, boss movement/attacks and boss-local hit validation remain explicit producers while the remaining non-trajectory actors are integrated.
...
The evolving `GameSession` contract is documented in [`GAME_SESSION.md`](GAME_SESSION.md). The session now owns trajectory encounter state, the exact Probe attach/decode/disarm chain, rapid-missile/Stinger Drone-hit producers, normal and destructive Drone objective execution through life loss/restart or EndRun, automatic shareware boss dispatch, and the established Lid/Top/Gemini lifecycle/score tails. Template-selection randomness, trajectory sprite-mask hits, attached-Probe vulnerability, hostile Stinger target selection, direct detonation framebuffer presentation, boss movement/attacks, exact boss-local collision validation and broader post-game continuity remain separate recovered producers or later Phase-4 milestones.
