...
The evolving `GameSession` contract is documented in [`GAME_SESSION.md`](GAME_SESSION.md). The session now owns trajectory encounter state, the exact Probe attach/decode/disarm chain plus enemy-bomb knockoff/phase-2 decoder interruption, rapid-missile/Stinger Drone-hit producers, normal and destructive Drone objective execution through life loss/restart or EndRun, automatic shareware boss dispatch, and the established Lid/Top/Gemini lifecycle/score tails. Immutable trajectory/Lid-Top sprite-mask pixels, non-owned hostile geometry/AI feeding the now-native Stinger selector, direct detonation framebuffer presentation, Gemini movement/exact local collision validation and broader post-game continuity remain separate recovered producers or later Phase-4 milestones; persistent primary replenishment, live transient trajectory template/timing randomness, and encounter-to-mission alien-statistics accounting are now session-owned.
