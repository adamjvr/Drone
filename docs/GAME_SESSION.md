...
1. advance the pre-phase Drone destruction countdown and, at exact count 99, trigger activity 2, commit outcome 2 and apply the dedicated -1000/progress-reset consequence; the same transition stops/rewinds `air.wav` immediately;
2. before the shared four-phase state-2 substep, raise the process-global `air.wav` scalar one unit toward 50 whenever Drone settlement is >=60; then advance the scheduler, logical Drone detonation tick (capped at 330) and phase-2 settlement scalar, and finally lower the air scalar one unit on phase-2 updates while settlement is <60;
...
7. before normal path advancement on phase 2, run the original non-primary mode-10 breakaway gate, including its unconditional-per-eligible-group `rand() % 300` consumption before progression/full-activation checks; initialize all fixed actor slots to the recovered 16.16 fly-off targets when the gate wins, then advance owned trajectory groups, stagger activation and mode-2 escape retirement; newly staggered non-primary actors increment both the encounter-local total and the mission-wide total immediately, preserving the original live double-count quirk before the later whole-encounter fold;
...
9. advance owned normal Drone travel/hover/settlement state together with its parameterized `drone.wav` control: Y=-117 starts the loop at volume 0, eligible phase-2 approach updates ramp the persistent scalar toward 80, transient Y=-40 emits one-shot `hintdron.wav` (volume 80, flags 0) and stores Y=-39, completed decode releases the Y=45 hold in the same update, and the exact 4200-tick unresolved hover stops the loop and starts the internal destructive countdown; after Y>230 the session clears completed decode state as the original does;
10. execute the recovered normal Drone interstitial/encounter-only transition when Y>230 and settlement reaches exactly 60, snapshotting encounter hit/miss/total/percentage and folding the complete encounter pair into the mission pair before encounter-only reset; both normal and destructive encounter rebuilds then perform the original `air.wav` `StopAndRewind -> Play(loop) -> SetVolume(0)` restart sequence;
11. derive the exact Drone Y=-200 boss boundary from owned position and select/advance Lid/Top or Gemini; each active boss preserves positive descent velocity when Y>=240, wraps fixed Y to -100, advances the shared traversal byte and requests its flags-0 `level1.wav`/`level2.wav` cue only on wrap 8;
...
14. advance/load/cycle/launch/move Probe/Stinger state; successful load resets the red-Stinger target to the original X=160 dummy; owned activity-1 trajectory actors then seed the same retained target pointer using the recovered >20-pixel vertical gate and strict Manhattan-like nearest-target metric, while frame-1 movement still runs the recovered Mothership > Gemini > Lid/Top > Spidey > registered-slot2 > Bomber > unresolved-dynamic-hostile priority chain and retains/overrides the shared pointer according to original ordering;
...
`GameSession::original_audio` carries both process-lifetime audio-control scalars currently native to gameplay: `0x00440278` for `drone.wav` and `0x004729A0` for `air.wav`. Neither is reset with encounter/campaign state. The exact Y=-117 Drone start writes zero before its first live ramp; the post-encounter air restart writes zero and explicitly restarts the loop before the next +1/update fade-up. Audio events remain semantic and asset-free: `SetVolume` carries the original 0..100 game-scale value while the eventual backend owns conversion/mixing.
...
- Stinger target-candidate activity/geometry facts only for hostile families whose movement/geometry is not session-owned; ordinary trajectory actors are now scanned directly from session-owned state, while selection priority and retained target state remain owned by the session;
...
- post-game pixel presentation, interactive high-score name editing and score-file persistence; the Results/Ordering/credits semantic audio lifetime and modal control flow are session-owned and the credits 99..0 fade is host-audio-owned;
...
Phase 5 emits allocation-free semantic audio events from exact gameplay and post-game ownership boundaries. Ordered late-bomb special/player-hit calls are produced inside the per-slot collision loop; exact trajectory/Gemini/Lid-Top impact sites advance the process-global `0x0042EFD8` `explode2/explode2/explode3/explode4` cycle. The two shareware-reachable boss loops are session-owned as well: Lid/Top starts `retro1.wav` at activation and stops it on the exposed-core Stinger destruction transition, while Gemini starts `gemini.wav` at activation and stops it only when the last activity-1 side crosses its damage threshold. Their traversal one-shots are native too: fixed-Y wrap count 8 emits flags-0 `level1.wav` (Lid/Top, volume 90) or `level2.wav` (Gemini, volume 100) and resets the shared cadence byte. `drone.wav` and the gameplay-owned `air.wav` now carry exact parameterized volume/lifetime events from their native session state; the two Drone-adjacent one-shots are also native, with `hintdron.wav` at the transient Y=-40 approach landmark and `parachut.wav` immediately before the successful decode stop; the air path preserves its pre-scheduler +1 fade, phase-2 -1 fade, detonation stop and post-encounter zero-volume restart. Results emits the exact selected one-shot cue; Ordering Information and completion credits emit loop-owned start/stop events. The DirectSound reverse layer also catalogs 13 proven flags-1 call sites (8 literal + 5 register-proven). `GameSession::original_audio` survives campaign/encounter resets, matching the original process-global lifetime.
