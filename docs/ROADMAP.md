...
**Current progress:** the asset-free audio runtime now owns exact DirectSound primitives, the original 20-voice reuse/steal policy, ordered impact-event multiplicity, process-global explosion-SFX phase, Results/Ordering/Credits ownership including the exact 99..0 credits fade, all 13 proven flags-1 start sites as metadata, native Lid/Top/Gemini encounter-loop start/stop events, the exact every-eighth-traversal `level1.wav`/`level2.wav` one-shot cadence, the parameterized `drone.wav` Y=-117 start/ramp/decode/interruption/stop controls, the gameplay-owned `air.wav` +1/-1 settlement envelope/detonation/restart, and a separate host runtime for `lowbees.wav`, the menu 11025-Hz air restart and states-5/6/99 air fade-to-stop/resume behavior. The formerly unresolved Drone one-shots are now native as `hintdron.wav` at Y=-40 and `parachut.wav` immediately before the decode-completion Drone-loop stop. Static trajectory initialization is now closed too: every `squad1.wav`..`squad14.wav` family is proven as a 20-voice pool at volume 80 with no initialization frequency override. The DOS HMI backend is now executable-backed too: canonical `DRONE_SW.EXE` configures exactly 32 x 0xF0 voices; StartSample chooses the first inactive voice and returns failure on saturation without priority-based stealing; volume is native packed HMI channel data; sustained loops use `wLoopCount=0xFFFFFFFF`; and controlled sounds retain voice handles for status/stop/volume/rate operations. The DOS presentation lifecycle is now closed too: Lowbees ownership is state-mapped, states 5/6/99 fade the HMI-wide master while Air remains continuously voiced, resume restores master volume without restarting Air, and gameplay teardown SampleDone/stops Air before menu ownership. `Q-AUDIO-003..007` are all resolved. The first portable original-backend contract is now implemented as well: `AudioEvent` parameters carry explicit domains; Win32 and DOS lower to distinct voice topologies, saturation rules, loop encodings, sample/master-volume representations and stop semantics; unsupported value domains and DOS-missing `hiphop`/`credits` cues are rejected rather than guessed. The host-independent original-policy voice runtime is now implemented too: Win32 per-cue 20-slot pools execute first-free/slot-0-steal semantics, DOS executes one global 32-voice first-free/fail-on-saturation array, stale completion handles are generation-guarded, and dedicated/retained sample controls plus the DOS digital master are modeled without sample data. The portable sample mixer/render path is now implemented too: canonical signed-16-bit mono/stereo samples use Q32.32 cursor stepping, integer linear interpolation, backend-native gain/master stages, exact full-sample looping, overlap summing/saturation, and natural-completion retirement into the generation-safe voice runtime. The next milestone is original-asset import/decode plus host-device binding, without moving callback timing into deterministic gameplay state.
